 ...
 * Auxiliary Space: O(n)
 */
public class A62_LargestSubarrayWithEqualNumberOf0sAnd1s {
	...
	private int[] getSubarray(int [] arr){
		...
		int n = arr.length, maxsize = -1, startindex = 0;  // variables to store result values

		// Create an auxiliary array sunmleft[] sumleft[i] will be sum of array elements from arr[0] to arr[i]
		...
		if ( maxsize == -1 )
			...
			result[0] = startindex;
			result[1] = startindex+maxsize-1;
		}
		return result;
	}

	public static void main(String[] args) {
		A62_LargestSubarrayWithEqualNumberOf0sAnd1s obj = new A62_LargestSubarrayWithEqualNumberOf0sAnd1s();
		int[] arr = {1,1,1, 1};
